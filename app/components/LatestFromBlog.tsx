...
export default async function LatestFromBlog() {
  ...
  return (
    <section className="pt-10 sm:pt-14">
      <h2 className="font-display border-b border-line pb-3 text-2xl font-bold tracking-tight text-ink">
        Latest
      </h2>
      <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {posts.map((p) => (
          <Link key={p.id} href={`/${p.slug}`} className="group block">
            ...
            <h3 className="font-display mt-1 text-lg font-bold leading-snug text-ink group-hover:text-accent">
              {p.title}
            </h3>
          </Link>
        ))}
